...
- Fixed cast buttons not appearing during ready check: the `OnEvent` handler was gated behind a cooldown check, so if the soulstone was on cooldown the cast buttons were never shown. Cast buttons were made unconditional at this point; this was later revised — see the final entry below.
- Fixed no prompt appearing during ready check when soulstone is on cooldown: the status message was only printed when `duration == 0`, so if the soulstone was still on cooldown the ready check fired silently with no notification. A cooldown-remaining message was added at this point; this approach was later revised — see the final entry below.
...
- Fixed addon being completely silent for Warlock players: `UnitClass()` in 3.3.5a returns only two values `(localizedName, englishClass)` — the numeric class index as a third return value did not exist until a later expansion. `classIndex` was therefore always `nil`, causing every warlock check (`classIndex == 9` / `classIndex ~= 9`) to silently fail and disabling the entire addon. Replaced all five occurrences with `englishClass == "WARLOCK"` / `englishClass ~= "WARLOCK"` and removed the unused `classIndex` variable.
- Fixed notification firing on every raid roster change: `PARTY_MEMBERS_CHANGED` fires whenever any player joins or leaves the group, not just when the local player joins. Added a `lastGroupSize` tracker so the notification only triggers when the player transitions from ungrouped (0 members) to grouped (>0 members).
- Fixed notification firing while in combat: join and zone-in events (`PARTY_MEMBERS_CHANGED`, `PLAYER_ENTERING_WORLD`) now check `UnitAffectingCombat("player")` and suppress the notification if the player is in combat. `READY_CHECK` is exempt since it is deliberately triggered by the raid leader.
- Fixed prompt appearing during ready check when soulstone is on cooldown: `get_player_buffs()` (which shows the cast buttons) was called unconditionally before the cooldown check, so cast buttons were always shown on `READY_CHECK` regardless of cooldown. Additionally, a cooldown-remaining message was always printed. Both the cast buttons and the notification are now suppressed entirely when the soulstone is on cooldown — the prompt only appears when the soulstone is actually ready to use.
